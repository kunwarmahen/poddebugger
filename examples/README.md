...
| [coder_demo.py](coder_demo.py) | The **Coder agent + sandbox** (`--coder`) — the proposal contract, the gate prompt showing the full script, deny-by-default, and hash-exact persistent pre-approval. Stubbed executor, fully offline. | `python coder_demo.py --demo` (offline) · live invocations in the docstring |
| [prompt_optimization_demo.py](prompt_optimization_demo.py) | **Offline prompt-pack optimization** — prompts as data, a score function, and a critic LLM whose edits are kept only if the eval score strictly improves. Scripted critic + fake scorer, fully offline. | `python prompt_optimization_demo.py --demo` (offline) · live invocations in the docstring |
